...
# --- 4. ANALYSIS FUNCTIONS (Updated for Dark Mode) ---
...
def plot_trend(df, coin):
    fig = px.line(df, x=df.index, y=coin, title=f"{coin} Price Action")
    fig.update_layout(template="plotly_dark", plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
    return fig

def plot_price_distribution(df, coin):
    fig = px.histogram(df, x=coin, nbins=50, title=f"{coin} Volatility Distribution", color_discrete_sequence=['#3b82f6'])
    fig.update_layout(template="plotly_dark", plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
    return fig
...
def plot_rolling_statistics(df, coin, window, stat, show_volatility):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.index, y=df[coin], name="Spot Price", line=dict(color="#3b82f6", width=1.5)))
    fig.add_trace(go.Scatter(x=stat_series.index, y=stat_series, name=f"{window}D {stat}", line=dict(color="#f59e0b", width=2)))
...
def plot_time_series_decomposition(df, coin):
    ...
    fig = make_subplots(rows=4, cols=1, shared_xaxes=True, subplot_titles=["Original", "Trend Component", "Seasonality", "Residuals"], vertical_spacing=0.07)
    fig.add_trace(go.Scatter(x=df.index, y=df[coin], name="Original", line=dict(color="#3b82f6")), row=1, col=1)
...
def main():
    ...
    st.caption("Deep Exploratory Analytics & Time-Series Decomposition")
    ...
    if analysis_type == "Descriptive analysis":
        ...
        with c2:
            ...
            m2.metric("Annualized Volatility", f"{vol:.2%}")
            
            st.info(f"Insight: {coin1} currently exhibits a {'positive' if stats.loc['skewness'][coin1] > 0 else 'negative'} skew in return distribution.")

    elif analysis_type == "Price trend & distribution":
        t1, t2 = st.tabs(["Trend Line", "Return Histogram"])
    ...
    elif analysis_type == "Rolling stats & volatility":
        w = st.select_slider("Rolling Window (Days)", options=[7, 14, 30, 60, 90], value=30)
        s = st.segmented_control("Calculation Method", options=["Mean", "Median", "Standard Deviation"], default="Mean")
        st.plotly_chart(plot_rolling_statistics(df, coin1, w, s, True), use_container_width=True)
        ...
        fig.update_layout(template="plotly_dark", plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', title="Indexed Performance Comparison (Base=1)")
