...
def get_model_leaderboard(all_metrics_df):
    if all_metrics_df.empty: return pd.DataFrame(), "No metrics found."
...
def get_top_correlations(top_corr_df, coin):
    ...
    pos = f[f["Type"] == "Positive"].head(3)
    neg = f[f["Type"] == "Negative"].head(3)
    res = f"**Correlations for {coin}:**\n\n"
    res += "🟢 **Positive:** " + ", ".join([f"{r['Coin']}" for _,r in pos.iterrows()])
    res += "\n\n🔴 **Negative:** " + ", ".join([f"{r['Coin']}" for _,r in neg.iterrows()])
    return res
